ci_template = {{ $input := .global.workflow.input }}
...
protocols:
  ...
  - dhcp_servers:
    {{ range $i, $dhcps := $protocols.dhcp_servers }}  - {{$dhcps}}{{ end -}}
  {{- else }}
  - dhcp_servers: []
  {{ end -}}
  {{- if index $protocols "dns_domains" }}
    dns_domains:
	  {{ range $i, $dnsdomains := $protocols.dns_domains }} - {{ $dnsdomains }}{{ end }}
  {{- else }}
    dns_domains: []
  {{ end -}}
  {{- if index $protocols "dns_servers" }}
    dns_servers:
    {{ range $i, $dnsservers := $protocols.dns_servers }} - {{ $dnsservers }}{{ end }}
  {{- else }}
    dns_servers: []
  {{ end -}}
  ...
  {{- if index $protocols "ntp_servers" }}
    ntp_servers:
    {{ range $i, $ntpservers := $protocols.ntp_servers }} - {{ $ntpservers }}{{ end }}
  {{- else }}
    ntp_servers: []
  {{ end -}}
netapp:
  ...
  {{- if index $netapp.autosupport "mail_hosts" }}
      mail_hosts:
        {{ range $i, $netapp_autosupport_mail_hosts := $netapp.autosupport.mail_hosts }}- {{ $netapp_autosupport_mail_hosts }}{{ end }}
  {{- else }}
      mail_hosts: []
  {{ end -}}
  ...
  {{- if index $netapp.autosupport "to_addresses" }}
      to_addresses:
        {{ range $i, $netapp_autosupport_to_address := $netapp.autosupport.to_addresses }}- {{ $netapp_autosupport_to_address }}{{ end }}
  {{- else }}
      to_addresses: []
  {{ end -}}
    snmp:
  ...
  {{- range $i, $cluster := $netapp.clusters }}
    ...
    {{- if index $cluster.nodes "data_ports" }}
          data_ports:
          {{ range $i, $netapp_data_ports := $cluster.nodes.data_ports}} - {{$netapp_data_ports}}{{ end }}
    {{- else }}
          data_ports: []
    {{ end -}}
    ...
    {{- if index $cluster.nodes "fcp_ports" }}
          fcp_ports:
            {{ range $i, $netapp_fcp_ports := $cluster.nodes.fcp_ports }}- {{ $netapp_fcp_ports }}{{ end }}
    {{- else }}
          fcp_ports: []
    {{ end -}}
    ...
    {{- if index $cluster.nodes "network_data" }}
            data:
              {{ range $i, $data := $cluster.nodes.network_data }}- {{ $data }}{{ end }}
    {{- else }}
            data: []
    {{ end -}}
...
intersight:
  ...
  {{- if index $imm "domains" }}
    ...
    {{ - range $i, $domain := $imm.domains}}
      ...
      {{- if index $domain "serial_numbers"}}
        serial_numbers:
          {{ range $i, $serial := $domain.serial_numbers }} - {{ $serial }}{{ end -}}
      {{- else }}
        serial_numbers: []
      {{ end -}}
        eth_uplink_ports:
          {{ range $i, $eth_uplink_ports := $domain.eth_uplink_ports }} - {{ $eth_uplink_ports }}{{ end }}
      ...
      {{- if index $domain "fcp_ports" }}
        fcp_uplink_ports:
          {{ range $i, $fcp_uplink_ports := $domain.fcp_ports }}- {{ $fcp_uplink_ports }}{{ end }}
      {{- else }}
        fcp_uplink_ports: []
      {{ end -}}
      ...
      {{- if index $domain "vsans" }}
        vsans:
          {{ range $i, $vsan := $domain.vsans }} - {{ $vsan }}{{ end }}
      {{- else }}
        vsans: []
      {{ end -}}
        network:
      {{- if index $domain "network_data" }}
          data:
            {{ range $i, $data := $domain.network_data }}- {{ $data }}{{ end }}
      {{- else }}
          data: []
      {{ end -}}
      ...
      {{ if index $domain "profiles" }}
        ...
        {{- range $i, $profile := $domain.profiles}}
          ...
          {{- if index $profile "domain_ports"}}
            domain_ports:
              {{ range $i, $domain_ports := $profile.domain_ports }}- {{ $domain_ports }}{{ end }}
          {{- else }}
            domain_ports: []
          {{ end -}}
  ...
  {{- if index $imm.policies.snmp "snmp_servers"}}
        servers:
          {{ range $i, $snmp_servers := $imm.policies.snmp.snmp_servers }}- {{ $snmp_servers }}{{ end }}
  {{- else }}
        servers: []
  {{ end -}}
      syslog:
  {{- if index $imm.policies "syslog_servers"}}
        servers: 
          {{ range $i, $syslog_servers := $imm.policies.syslog_servers }}- {{ $syslog_servers }}{{ end }}
  {{- else }}
        servers: []
  {{ end -}}
      virtualization:
  ...
  {{- if index $imm.virtualization "virtual_switches" }}
    ...
    {{- range $i, $switch := $imm.virtualization.virtual_switches }}
      {{- if index $switch "data_types"}}
            - data_types:
                {{range $i, $data_types := $switch.data_types}}- {{$data_types}}{{ end }}
      {{- else }}
            - data_types: []
      {{ end -}}
  ...
  {{- if index .global.workflow.input.nxos "switches" }}
    ...
    {{- range $i, $switch := .global.workflow.input.nxos.switches }}
      ...
      {{- if index $switch "vpc_keepalive_ports" }}
          keepalive_ports:
            {{ range $i, $vpc_keepalive_ports := $switch.vpc_keepalive_ports }}- {{ $vpc_keepalive_ports }}{{ end }}
      {{- else }}
          keepalive_ports: []
      {{ end -}}
      {{- if index $switch "vpc_peer_ports"}}
          peer_ports:
            {{ range $i, $vpc_peer_ports := $switch.vpc_peer_ports }}- {{ $vpc_peer_ports }}{{ end }}
      {{- else }}
          peer_ports: []
      {{ end -}}
    {{ end -}}
