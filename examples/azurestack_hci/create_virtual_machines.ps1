...
Write-Host "Beginning Process to Create Virtual Machines on $cluster" -ForegroundColor Yellow
$results = Invoke-Command $cluster -Credential $credential -scriptblock {
    ...
    if ($wsman_creds -match "This computer is configured to receive credentials from a remote client computer") {
        ...
        Write-Host "To Determine Why the $($Using:cluster) Failed to Enable CredSSP." -ForegroundColor Red
...
if ($results.Completed -eq $True) { 
    $results = Invoke-Command $cluster -Credential $credential -authentication Credssp -scriptblock {
        ...
        Write-Host "Completed Process to Create Virtual Machines on $cluster" -ForegroundColor Yellow
        Return New-Object PsObject -property @{Completed=$True}
    }
}
Stop-Transcript
if ($results.Completed -eq $True) { 
    ...
    Write-Host "Failed to Complete the Process to Create Virtual Machines on $cluster.  Review the Logs to Investigate." -ForegroundColor Red
    Exit 1
}
